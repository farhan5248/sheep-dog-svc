...
Feature: Metrics Csv Timestamps
  ...
  \@darmok-maven-plugin
  The `metrics.csv` file that Darmok appends after every successful scenario holds one column ? `Timestamp` ? that Grafana reads as the x-axis for the XmR process-behaviour charts. For Grafana's Infinity datasource to place a row at the right point in time, the timestamp must be unambiguous: a naive local-time string is interpreted as UTC by the parser, and the dashboard's `timezone: browser` conversion then shifts every point by the local offset (e.g. a 14:32 run renders at 10:32 in a UTC-4 browser). Emitting a timezone-aware ISO-8601 timestamp `yyyy-MM-dd'T'HH:mm:ss.SSSXXX` lets the parser resolve the instant without guessing. Issue 316.

  @GH316
  Scenario: Timestamp is written in timezone-aware ISO format

    \@GH316
    The metrics.csv row emitted after a scenario's RGR cycle holds a Timestamp column whose value parses as ISO-8601 with a numeric offset suffix. The shape is pinned via the `Timestamp` placeholder ? any value that fails to parse as `yyyy-MM-dd'T'HH:mm:ss.SSSXXX` fails this assertion.
