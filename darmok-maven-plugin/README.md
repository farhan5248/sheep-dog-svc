...
The observability Helm chart (`sheep-dog-ops/observability/`) runs a Grafana pod that reads `metrics.csv` from a hostPath-backed PVC. To route your Darmok run's CSV into that pipeline instead of the per-project `target/darmok/` directory, point `LOG_PATH` at the shared folder before running the plugin.
...
This directory is mounted into the minikube VM at `/mnt/darmok-metrics` via the `minikube start --mount-string` branch in `sheep-dog-ops/minikube/setup-cluster-local.bat`. If minikube is already running without that mount, `minikube delete` and rerun `setup-cluster-local.bat` to pick it up.
...
Open http://localhost:3000, log in as `admin` (password is the plaintext value in the `observability-grafana` Secret — `kubectl -n observability get secret observability-grafana -o jsonpath="{.data.admin-password}" | base64 -d`), and navigate to **Dashboards → Darmok RGR cycle time (SPC)** (direct: http://localhost:3000/d/darmok-spc).
